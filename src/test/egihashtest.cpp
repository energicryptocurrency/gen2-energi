...
static inline void SHA3_256(struct egihash_h256 const* ret, uint8_t const* data, size_t const size)
{
...
std::string blockhashToHexString(egihash_h256_t* _hash)
{
...
egihash_h256_t stringToBlockhash(std::string const& _s)
{
	egihash_h256_t ret;
...
BOOST_AUTO_TEST_CASE(SHA256_check)
{
	...
	const std::string expected = "2b5ddf6f4d21c23de216f44d5e4bdc68e044b71897837ea74c83908be7037cd7";
	const std::string actual = bytesToHexString((uint8_t*)&out, 32);
...
//		egihash_io_prepare("./test_egihash_directory/", seedhash, &f, 64, false)
//	);
//	BOOST_REQUIRE(f);
//
//	// let's make sure that the directory was created
//	BOOST_REQUIRE(fs::is_directory(fs::path("./test_egihash_directory/")));
//
//	// cleanup
//	fclose(f);
...
//}
//
//BOOST_AUTO_TEST_CASE(test_egihash_io_memo_file_match) {
//	uint64_t full_size;
//	uint64_t cache_size;
//	egihash_h256_t seed;
//	egihash_h256_t hash;
...
//	full_size = 1024 * 32;
//
//	egihash_light_t light = egihash_light_new_internal(cache_size, &seed);
//	egihash_full_t full = egihash_full_new_internal(
//		"./test_egihash_directory/",
//		seed,
//		full_size,
//		light,
//		NULL
...
//	egihash_full_delete(full);
//	// and check that we have a match when checking again
//	BOOST_REQUIRE_EQUAL(
//		ETHASH_IO_MEMO_MATCH,
//		egihash_io_prepare("./test_egihash_directory/", seed, &f, full_size, false)
//	);
//	BOOST_REQUIRE(f);
//
//	// cleanup
...
//}
//
//BOOST_AUTO_TEST_CASE(test_egihash_io_memo_file_size_mismatch) {
//	static const int blockn = 0;
//	egihash_h256_t seedhash = egihash_get_seedhash(blockn);
//	FILE *f = NULL;
//	BOOST_REQUIRE_EQUAL(
//		ETHASH_IO_MEMO_MISMATCH,
//		egihash_io_prepare("./test_egihash_directory/", seedhash, &f, 64, false)
...
//		egihash_io_prepare("./test_egihash_directory/", seedhash, &f, 65, false)
//	);
//
//	// cleanup
//	fs::remove_all("./test_egihash_directory/");
//}
//
//BOOST_AUTO_TEST_CASE(test_egihash_get_default_dirname) {
//	char result[256];
...
//	BOOST_REQUIRE(egihash_get_default_dirname(result, 256));
//	std::string res = std::string(homedir) + std::string("\\AppData\\Local\\Ethash\\");
//#else
//	char* homedir = getenv("HOME");
//	BOOST_REQUIRE(egihash_get_default_dirname(result, 256));
//	std::string res = std::string(homedir) + std::string("/.egihash/");
//#endif
...
//	egihash_h256_t seed;
//	egihash_h256_t hash;
//	egihash_h256_t difficulty;
//	egihash_return_value_t light_out;
//	egihash_return_value_t full_out;
//	memcpy(&seed, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);
//	memcpy(&hash, "~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);
//
//	// Set the difficulty
//	egihash_h256_set(&difficulty, 0, 197);
//	egihash_h256_set(&difficulty, 1, 90);
//	for (int i = 2; i < 32; i++)
//		egihash_h256_set(&difficulty, i, 255);
//
//	cache_size = 1024;
//	full_size = 1024 * 32;
//
...
//	{
//		node node;
//		egihash_calculate_dag_item(&node, 0, light);
//		const std::string
...
//				node expected_node;
//				egihash_calculate_dag_item(&expected_node, j, light);
//				const std::string
...
//		full_out = egihash_full_compute(full, hash, nonce);
//		BOOST_REQUIRE(full_out.success);
//		light_out = egihash_light_compute_internal(light, full_size, hash, nonce);
...
//		egihash_h256_t check_hash;
//		egihash_quick_hash(&check_hash, &hash, nonce, &full_out.mix_hash);
...
//	}
//	{
//		full_out = egihash_full_compute(full, hash, 5);
//		BOOST_REQUIRE(full_out.success);
//		std::string
//				light_result_string = blockhashToHexString(&light_out.result),
//				full_result_string = blockhashToHexString(&full_out.result);
...
//
//		light_out = egihash_light_compute_internal(light, full_size, hash, 5);
//		BOOST_REQUIRE(light_out.success);
//		light_result_string = blockhashToHexString(&light_out.result);
//		BOOST_REQUIRE_MESSAGE(light_result_string == full_result_string,
...
//				"\nlight mix hash: " << light_mix_hash_string.c_str() << "\n"
//						<< "full mix hash: " << full_mix_hash_string.c_str() << "\n");
//		BOOST_REQUIRE_MESSAGE(egihash_check_difficulty(&full_out.result, &difficulty),
//				"egihash_check_difficulty failed"
//		);
//		BOOST_REQUIRE_MESSAGE(egihash_quick_check_difficulty(&hash, 5U, &full_out.mix_hash, &difficulty),
//				"egihash_quick_check_difficulty failed"
//		);
//	}
//	egihash_light_delete(light);
//	egihash_full_delete(full);
...
//}
//
//BOOST_AUTO_TEST_CASE(egihash_full_new_when_dag_exists_with_wrong_size) {
//	uint64_t full_size;
//	uint64_t cache_size;
//	egihash_h256_t seed;
//	egihash_h256_t hash;
...
//	BOOST_REQUIRE_EQUAL(
//		ETHASH_IO_MEMO_MISMATCH,
//		egihash_io_prepare("./test_egihash_directory/", seed, &f, 64, false)
//	);
//	fclose(f);
//
...
//	egihash_light_t light = egihash_light_new(60000);
//	egihash_h256_t seedhash = stringToBlockhash("5fc898f16035bf5ac9c6d9077ae1e3d5fc1ecc3c9fd5bee8bb00e810fdacbaa0");
//	BOOST_ASSERT(light);
//	auto ret = egihash_light_compute(
//		light,
//		seedhash,
...
BOOST_AUTO_TEST_CASE(full_dag_test) {
	egihash_light_t light = egihash_light_new(55);
	BOOST_ASSERT(light);
	egihash_full_t full = egihash_full_new(light, progress_cb);
	BOOST_ASSERT(full);
	egihash_light_delete(light);
	egihash_full_delete(full);
}
