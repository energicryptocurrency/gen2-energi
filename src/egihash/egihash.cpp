...
namespace
{
	namespace constants
	{
		...
		constexpr EGINS(h256_t) empty_h256 = {{0}};
		constexpr EGINS(result_t) empty_result = {{{0}}, {{0}}};
	}
	...
	struct sha3_256_t : public sha3_base<32, ::sha3_256>
	{
		...
		sha3_256_t(EGINS(h256_t) const & h256)
		: sha3_base()
		{
...
namespace egihash
{
	...
	// TODO: unit tests / validation
	::std::vector<sha3_512_t::deserialized_hash_t> calc_dataset(::std::vector<sha3_512_t::deserialized_hash_t> const & cache, size_t const full_size, EGINS(callback) progress_callback)
	{
...
extern "C"
{
	struct EGINS(light)
	{
		...
		EGINS(light)(unsigned int block_number)
		...
		EGINS(result_t) compute(EGINS(h256_t) header_hash, uint64_t nonce)
		{
			// TODO: copy-free version
			EGINS(result_t) result;
	...
	struct EGINS(full)
	{
		...
		EGINS(full)(EGINS(light_t) light, EGINS(callback) callback)
		: light(light)
	...
	EGINS(light_t) EGINS(light_new)(unsigned int block_number)
	{
		try
		{
			return new EGINS(light)(block_number);
		}
		catch (...)
		{
	...
	EGINS(full_t) EGINS(full_new)(EGINS(light_t) light, EGINS(callback) callback)
	{
		try
		{
			return new EGINS(full)(light, callback);
		}
		catch (...)
		{
