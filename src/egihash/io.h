/*
  This file is part of egihash.

  egihash is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  egihash is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU General Public License for more details.

  You should have received a copy of the GNU General Public License
  along with egihash.  If not, see <http://www.gnu.org/licenses/>.
*/
/** @file io.h
...
// Reference: https://github.com/ethereum/wiki/wiki/egihash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see egihash_io_prepare
enum egihash_io_rc {
	egihash_IO_FAIL = 0,           ///< There has been an IO failure
	egihash_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	egihash_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	egihash_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of egihash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * egihash_full_t
 */
#ifdef egihash_PRINT_CRITICAL_OUTPUT
#define egihash_CRITICAL(...)							\
	do													\
	{													\
		printf("egihash CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for egihash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the egihash
 ...
 *                           https://github.com/ethereum/wiki/wiki/egihash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum egihash_io_rc
 */
enum egihash_io_rc egihash_io_prepare(
	char const* dirname,
	egihash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* egihash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* egihash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool egihash_mkdir(char const* dirname);
 ...
 */
bool egihash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* egihash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereum/wiki/wiki/egihash-DAG
 *
 ...
 */
bool egihash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool egihash_io_mutable_name(
	uint32_t revision,
	egihash_h256_t const* seed_hash,
	char* output
)
{
    uint64_t hash = *((uint64_t*)seed_hash);
#if LITTLE_ENDIAN == BYTE_ORDER
    hash = egihash_swap_u64(hash);
#endif
